...
std::map<int,int> idxprom_nodeID_pred_map;  //Map containing idxprom nodes and predecessor
   ...
   /* Instruction* Find_Ret_Instruction(BasicBlock* bb)
	 ...
	 /* node = new REGI_NODE(add, 1, MahdiNodeID++, BI->getName().str(), BI); //mahesh jan'16
	  ...
	  node = new REGI_NODE(div, 1, MahdiNodeID++, BI->getName().str(), BI);
        ...
        case Instruction::GetElementPtr:
	        node = new REGI_NODE(add, 1, MahdiNodeID++, BI->getName().str(),BI);
	        mynodenamefile << BI->getDebugLoc().getLine() << "\n";
 	        myDFG->insert_Node(node);
          retVal = true;
          return retVal;
        ...
        case Instruction::SExt: //TODO Consider optimization related to this instructions and idxprom node
   	      if(BI->getName().str().find("idxprom") != std::string::npos)
	         node = new REGI_NODE(mult, 1, MahdiNodeID++, BI->getName().str(),BI);
	        else
	         node = new REGI_NODE(add, 1, MahdiNodeID++, BI->getName().str(),BI);
 	        myDFG->insert_Node(node);
      ...
      for (iNode = succ_node.begin(); iNode != succ_node.end(); iNode++)
	{
     ...
     /*if (BI->getOpcode() == Instruction::PHI)
      ...
      //For 32 Bit Arithmetic We Dont need To Extend (Can Skip Currently)
      if((BI->getOpcode() == Instruction::SExt) || (BI->getOpcode() == Instruction::ZExt))
      {
  	     node1 = myDFG->get_Node(BI);
	       if(node1->get_Name().find("idxprom") != std::string::npos)
	        node2 = new REGI_NODE(constant, 1, MahdiNodeID++, "ConstInt4", NULL);
	      else
	        node2 = new REGI_NODE(constant, 1, MahdiNodeID++, "ConstInt0", NULL);

	      myDFG->insert_Node(node2);
        myDFG->make_Arc(node2, node1, MahdiEdgeID++, 0, TrueDep, 1);
	      //mynodenamefile << node1->get_Name() << "\t" << BI->getOperand(0)->getName().str()   << "\n";
      }
   ...
   for(unsigned int i=0; i< BI->getNumOperands(); ++i)
    {
    	...
    	//Add Node If Unnamed Private Constant
    	if(operandname.find("main.") != std::string::npos)
    	{
    	  std::string str = BI->getOperand(i)->getName().str();
    	  str.erase(str.begin(),str.begin()+5); //Erase main. from name
    	  //mynodenamefile << str << "\n";
    	  node2 = new REGI_NODE(constant, 1, MahdiNodeID++, str , NULL);
    	  myDFG->insert_Node(node2);
    	  myDFG->make_Arc(node2, node1, MahdiEdgeID++, 0, TrueDep, 0);
    	}
    	else
      ...
      if(i==2)
      {
      	...
      	if(node->get_Name().find("idxprom") != std::string::npos)
      	{
          ...
          //if(idxprom_nodeID_pred_map.find(node->get_ID()) == idxprom_nodeID_pred_map.end())
            //idxprom_nodeID_pred_map.insert(std::pair<int,int>(node->get_ID(),node));
        	break;
      	}
        ...
        if(idxprom_nodeID_pred_map.find(node->get_ID()) != idxprom_nodeID_pred_map.end())
        {
          ...
          node3 = myDFG->get_Node(idxprom_nodeID_pred_map[node->get_ID()]);
        	myDFG->make_Arc(node, node3, MahdiEdgeID++, 0, TrueDep, 0);
        ...
        else
        {
          ...
          node4 = new REGI_NODE(constant, 1, MahdiNodeID++, "ConstInt4", NULL);
  	      ...
  	      //mynodenamefile << node1->get_Name() << "\t" << node->get_Name() << "\t" << node3->get_Name() << "\t" << node4->get_Name() << "\t" << node2->get_Name() << "\n";
        	arc1 = myDFG->get_Arc(node, node1);
        	myDFG->Remove_Arc(arc1);
        	myDFG->make_Arc(node, node3, MahdiEdgeID++, 0, TrueDep, 0);
...
virtual bool runOnLoop(Loop *L, LPPassManager &LPM)
{
      ...
      std::cout << "Shail\tDepth=" << L->getLoopDepth() << "\tNumber of BBs=" << bbs.size() << " With one exiting branch\n";
      std::cout << "Shail\t" << TOTALLOOPS << "\n";
      TOTALLOOPS++;
